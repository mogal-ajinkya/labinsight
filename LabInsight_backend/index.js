...
// db connection 
mongoose.connect('mongodb://0.0.0.0:27017/Labinsights', {
...
app.post('/network_details', async (req, res) => {
  try {
    ...
    const collectionName = `network_details_${source_ip}`;
...
// for client admin -> for montioring 
...
app.get('/network_details/:ipAddress', async (req, res) => {
  try {
    ...
    const collectionName = `network_details_${ipAddress}`;
    const NetworkDetails = mongoose.model(collectionName, NetworkDetailsSchema, collectionName);
