...
const summaryCards = () => {
    ...
    //     const networkResponse = await fetch(`http://localhost:4141/network_details/${ipAddress}`);
        
    //     const networkData = await networkResponse.json();
    //     setNetworkDetails(networkData);
        
    //     // Fetch process details
    //     const processResponse = await fetch(`http://localhost:4141/process_details/${ipAddress}`);
    //     const processData = await processResponse.json();
    //     setProcessDetails(processData);
  
    //     // Fetch connected devices
    //     const devicesResponse = await fetch(`http://localhost:4141/connected_devices/${ipAddress}`);
    ...
    // const calculateStatistics_network = (data) => {
    //     let statistics = {};
    
      
    //       // Calculate statistics for network details
    //       if (data && data.length > 0) { // Check if data is not null and has length greater than 0
    //         statistics.totalRecords = data.length;
    //         // Calculate other statistics...
    //       }
    //       statistics.uniqueSourceIPs = new Set(data.map(item => item.source_ip)).size;
    //       statistics.uniqueDestinationIPs = new Set(data.map(item => item.destination_ip)).size;
    //       // Calculate other statistics...
       
    
    //     return statistics;
    //   };
    //   const calculateStatistics_process = (data) => {
    //     let statistics = {};
    
      
    //       // Calculate statistics for network details
    //       if (data && data.length > 0) { // Check if data is not null and has length greater than 0
    //         statistics.totalProcesses = data.length;
    //         // Calculate other statistics...
    //       }
    //   statistics.uniqueUsers = new Set(data.map(item => item.Username)).size;
    //   statistics.averageCPUUsage = data.reduce((sum, item) => sum + item['CPU Percent'], 0) / data.length;
    //       // Calculate other statistics...
    ...
    //     return statistics;
    //   };
    //   const calculateStatistics_connecteddevices = (data) => {
    //     let statistics = {};
    
      
    //       // Calculate statistics for network details
    //       if (data && data.length > 0) { // Check if data is not null and has length greater than 0
    //         statistics.totalDevices = data.length;
    //         // Calculate other statistics...
    //       }
    //       statistics.uniqueDeviceTypes = new Set(data.map(item => item.deviceType)).size;
    //       // Calculate other statistics...
       
    
    //     return statistics;
    //   };
    // const networkdetails_stats = calculateStatistics_network(networkDetails);
    // const processdetails_stats = calculateStatistics_process(processDetails);
    // const connecteddevdetails_stats = calculateStatistics_connecteddevices(connectedDevices);


    return (
        <div class="container">
        <h1 class="container-title"> { ipAddress }</h1>
      
        <div class="gradient-cards">
            <Link to={{pathname:`/network-details/${ipAddress}`}}>
          <div class="card">
            <div class="container-card bg-green-box">
              ...
              <p class="card-title">NETWORK DETAILS</p>
          ...
          <Link to={{pathname:`/process-details/${ipAddress}`}}>
          <div class="card">
            <div class="container-card bg-white-box">
              ...
              <p class="card-title">PROCESS DETAILS</p>
          ...
          <div class="card">
            <div class="container-card bg-yellow-box">
              ...
              <p class="card-title">BROWSER LOGS</p>
          ...
          <Link to={{pathname:`/connected-devices/${ipAddress}`}}>
          <div class="card">
            <div class="container-card bg-blue-box">
              ...
              <p class="card-title">CONNECTED DEVICES</p>
